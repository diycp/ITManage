...
class SendEmailCommand extends CConsoleCommand
{
    ...
    {
        try {
            Yii::log('[群发任务邮件]：'. $id, 'info', 'system.console');
            $email = new EmailComponent();
            ...
            foreach ($emails as $row) {
                ...
                $result = $email->sendEmail($receiver, $title, $content);
                Yii::log('[群发任务邮件]：'. $result, 'info', 'system.console');
            }
        } catch (Exception $e) {
...
// php /home/itmanage/ITManage/webroot/backstage/protected/yiic.php sendemail checkduty
